...
function ImportAndExport() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!file) {
      ...
      setUploadStatus({ type: 'error', message: 'Only .xlsx files are allowed!' });
      return;
    }
  ...
  return (
    <div className="import-export-container">
      <div className="import-export-card">
        ...
        <button className="import-export-button" onClick={() => handleDownload('https://docs.google.com/spreadsheets/d/1Vr3u5114jycAwCUDEvrkbtns8fZHOs4R/export?format=xlsx', 'แบบฟอร์มหลักสูตร.xlsx')}>
          ดาวโหลดแบบฟอร์ม
        </button>
      </div>
      <div className="import-export-card">
        ...
        <form className="file-input-container" onSubmit={handleSubmit}>
          <label htmlFor="file-input" className="input-file-button">Choose File</label>
          <input id="file-input" type="file" className="file-input" onChange={(e) => setFile(e.target.files[0])} />
          <button type="submit" className="import-export-button">Upload</button>
        </form>
      </div>
      <div className="import-export-card">
        ...
        <button className="import-export-button" onClick={handleDownloadTeachingSchedule}>
